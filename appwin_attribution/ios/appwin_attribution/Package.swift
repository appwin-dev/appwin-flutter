...
let package = Package(
  ...
  products: [
    ...
    .library(name: "appwin-support", targets: ["appwin_attribution"])
  ],
  dependencies: [
    // Provided by the Flutter tooling at build time.
    .package(name: "FlutterFramework", path: "../FlutterFramework"),
    // The native SDK lives in the same monorepo, as one package with four
    // products. The folder is named after the repository it is published to, so
    // the package identity (`appwin-ios`) is the same locally and once
    // published: the release script only swaps the path for a tagged git URL,
    // it never has to rewrite the `package:` labels below.
    .package(url: "https://github.com/appwin-dev/appwin-ios.git", from: "0.7.0"),
  ],
  targets: [
    .target(
      name: "appwin_attribution",
