#
# CocoaPods podspec for the `appwin_analytics` Flutter wrapper.
#
# Firebase/FlutterFire pattern (ADR-0020): this plugin compiles **only** its
# glue file. The native `AppwinCore` and `AppwinAnalytics` SDKs are declared as
# CocoaPods dependencies, which makes them real Swift modules, so
# `import AppwinCore` and `import AppwinAnalytics` work normally.
#
# Development, in the Flutter host app's Podfile:
#
#   target 'Runner' do
#     pod 'AppwinCore',    :path => '../../../core/AppwinCore'
#     pod 'AppwinAnalytics', :path => '../../AppwinAnalytics'
#     # ...
#   end
#
...
#
Pod::Spec.new do |s|
  s.name             = 'appwin_analytics'
  s.version          = '0.0.1'
  s.summary          = 'Flutter wrapper for the native iOS Appwin Analytics SDK.'
  s.description      = <<-DESC
Flutter plugin - a thin layer over the native AppwinCore and AppwinAnalytics SDKs
...
method channel.
  ...
  s.source_files = 'appwin_analytics/Sources/appwin_analytics/**/*.{h,m,swift}'
  ...
  s.dependency 'AppwinAnalytics', '>= 0.7.0', '< 1.0.0'
  ...
  s.resource_bundles = {'appwin_analytics_privacy' => ['appwin_analytics/Sources/appwin_analytics/PrivacyInfo.xcprivacy']}
end
