...
class AppwinSupportPlugin :
   ...
   */
  private var didTrackAppOpenThisForeground = false
  ...
  override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
    when (call.method) {
      ...
      "initialize" -> {
        ...
        // Wait for the session to open before handing back, as on iOS, so the
        // first product call that follows finds a token.
      ...
      "fetchPendingInAppMessages" -> launch(result, "fetch_in_app_failed") {
        // App-open automations only create a delivery after the event, so it is
        // sent before the read; otherwise the message it triggers would only
        // arrive on the next call.
        if (!didTrackAppOpenThisForeground) {
          didTrackAppOpenThisForeground = true
          runCatching { AppwinNotifications.trackEvent(AutomationEvent.APP_OPEN) }
        }
        result.success(AppwinNotifications.fetchPendingMessages().map(::serialize))
      }

      "trackInAppDelivery" -> {
        val deliveryId = call.argument<String>("deliveryId")
        val event = call.argument<String>("event")?.let { wire ->
          TrackEvent.entries.firstOrNull { it.wireValue == wire }
        }
        if (deliveryId.isNullOrBlank() || event == null) {
          result.error("bad_args", "deliveryId/event manquant", null)
          return
        }
        launch(result, "track_in_app_failed") {
  ...
  private fun serialize(message: InAppMessage): Map<String, Any?> = mapOf(
    "id" to message.id,
