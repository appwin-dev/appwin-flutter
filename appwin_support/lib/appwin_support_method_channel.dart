...
class AppwinSupportMethodChannel extends AppwinSupportPlatform {
  ...
  @override
  Future<List<AppwinInAppMessage>> fetchPendingInAppMessages() async {
    final raw = await methodChannel.invokeMethod<List<dynamic>>(
      'fetchPendingInAppMessages',
    );
    if (raw == null) return [];
    return raw
        .whereType<Map>()
        .map((m) => AppwinInAppMessage.fromMap(Map<dynamic, dynamic>.from(m)))
        .toList();
  }
  ...
  Future<void> trackInAppDelivery({
    ...
    await methodChannel.invokeMethod<void>('trackInAppDelivery', {
