...
public class AppwinNotificationsPlugin: NSObject, FlutterPlugin {
  private static let instance = AppwinNotificationsPlugin()
  ...
  /// Ensures push tap tracking works with Firebase Messaging (registers before / after it).
  ...
  @objc public func application(
    ...
    Task {
      await AppwinNotifications.application(
        application,
