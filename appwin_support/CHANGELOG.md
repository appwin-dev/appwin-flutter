...
- `updateUser()` takes a `plan`, to segment customers by subscription tier
  without borrowing another field for it.
- **Fix.** `loginUnidentifiedUser()` no longer invents a display name on
  Android. It sent `Visitor anonyme <deviceId>`, which then showed in the
  dashboard as though the customer had chosen it.
- Native SDKs at 0.5.0, which add the in-app banner raised when an agent
  replies while the customer is on another screen.

## 0.4.1

- Documentation on the whole public API, up from 55%: what the product is, the
  two ways to open a session, and every member of the platform interface and
  the method channel.
